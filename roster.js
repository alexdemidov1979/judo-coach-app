  ...
  // ================= ПУТЬ СПОРТСМЕНА (шкала освоения базовых навыков) =================
  // Это тренерская оценка, не медицинская/научная диагностика.
  const SKILL_PATH_ITEMS = [
    {key:'ukemi', label:'Ukemi', hint:'страховки при падении'},
    {key:'kuzushi', label:'Kuzushi', hint:'выведение из равновесия'},
    {key:'tsukuri', label:'Tsukuri', hint:'вход в приём'},
    {key:'kake', label:'Kake', hint:'завершение броска'},
    {key:'tachiwaza', label:'Tachi-waza', hint:'техника в стойке'},
    {key:'newaza', label:'Ne-waza', hint:'техника в партере'},
    {key:'randori', label:'Randori', hint:'учебная схватка'},
  ];
  const SKILL_PATH_STATES = ['none','red','yellow','green'];
  const SKILL_PATH_ICON = {none:'⚪', red:'🔴', yellow:'🟡', green:'🟢'};
  const SKILL_PATH_TITLE = {none:'ещё не изучалось', red:'проблема', yellow:'в процессе', green:'освоено'};
  function skillPathBadgeHtml(item, state){
    state = state || 'none';
    return `<div class="skill-badge" data-skill="${item.key}" data-state="${state}" title="${item.label} — ${SKILL_PATH_TITLE[state]}"
      style="display:flex;flex-direction:column;align-items:center;gap:2px;padding:6px 4px;border-radius:8px;background:var(--surface-2,var(--line));cursor:pointer;min-width:56px;">
      <span style="font-size:18px;">${SKILL_PATH_ICON[state]}</span>
      <span style="font-size:10px;color:var(--dim);text-align:center;">${item.label}</span>
    </div>`;
  }
  function skillPathBlockHtml(r){
    const sp = r.skillPath || {};
    return `<div class="skill-path-block" style="display:flex;flex-wrap:wrap;gap:6px;">
      ${SKILL_PATH_ITEMS.map(item=> skillPathBadgeHtml(item, sp[item.key])).join('')}
    </div>`;
  }

  const HOMEWORK_PRESETS = ['Ukemi (страховки)', 'Tai sabaki', 'Shadow uchikomi', 'Посмотреть видео техники'];
  function homeworkBlockHtml(r, i){
    const hw = (r.homework||[]).slice().reverse();
    return `
      ${hw.length ? hw.map(h=>`
        <div style="display:flex;align-items:flex-start;gap:6px;padding:4px 0;border-bottom:1px dashed var(--line);font-size:13px;">
          <input type="checkbox" class="hw-check" data-i="${i}" data-hid="${h.id}" ${h.done?'checked':''} style="margin-top:3px;">
          <div style="flex:1;${h.done?'text-decoration:line-through;color:var(--dim);':''}">${escapeHtml(h.text)}<div style="font-size:11px;color:var(--dim);">${h.date}</div></div>
          <button class="del hw-del" data-i="${i}" data-hid="${h.id}">✕</button>
        </div>
      `).join('') : '<div class="empty-hint">Заданий пока нет</div>'}
      <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;">
        ${HOMEWORK_PRESETS.map(p=>`<button class="btn small ghost hw-preset" data-i="${i}" data-text="${p.replace(/"/g,'&quot;')}">+ ${p}</button>`).join('')}
      </div>
      <div style="display:flex;gap:6px;margin-top:6px;">
        <input type="text" class="hw-custom" data-i="${i}" placeholder="своё задание..." style="flex:1;">
        <button class="btn small hw-add-custom" data-i="${i}">Добавить</button>
      </div>
    `;
  }

  // ================= ТРЕНЕРСКИЙ РАДАР («Требуют внимания») =================
  let radarFilter = null; // null | 'red' | 'yellow' | 'green'
  function attentionStatus(r, attStats){
    const a = attStats[r.name];
    const total = a ? a.present + a.absent : 0;
    const pct = total > 0 ? (a.present/total)*100 : null;
    const sp = r.skillPath || {};
    const hasRedSkill = SKILL_PATH_ITEMS.some(it=>sp[it.key]==='red');
    if(hasRedSkill || (pct!==null && pct<50)) return 'red';
    if((pct!==null && pct<80)) return 'yellow';
    return 'green';
  }
  function radarBarHtml(list, attStats){
    const counts = {red:0, yellow:0, green:0};
    list.forEach(r=> counts[attentionStatus(r, attStats)]++);
    const chip = (key, icon)=> `<div class="radar-chip" data-radar="${key}" style="cursor:pointer;padding:6px 10px;border-radius:8px;background:var(--surface-2,var(--line));${radarFilter===key?'outline:2px solid var(--navy,var(--belt-red));':''}">${icon} <b>${counts[key]}</b></div>`;
    return `<div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap;align-items:center;">
      <span style="font-size:12px;color:var(--dim);font-weight:700;">ТРЕБУЮТ ВНИМАНИЯ:</span>
      ${chip('red','🔴')} ${chip('yellow','🟡')} ${chip('green','🟢')}
      ${radarFilter ? `<button class="btn small ghost radar-clear">✕ сбросить фильтр</button>` : ''}
    </div>`;
  }
  ...
  async function renderRoster(){
    ...
    const radarHtml = radarBarHtml(list, attStats);
    const visibleCount = radarFilter ? list.filter(r=>attentionStatus(r,attStats)===radarFilter).length : list.length;
    el.innerHTML = radarHtml + (radarFilter && visibleCount===0 ? '<div class="empty-hint">Никого с таким статусом нет 🎉</div>' : '') + list.map((r,i)=>{
      if(radarFilter && attentionStatus(r, attStats)!==radarFilter) return '';
      ...
      <div class="roster-row">
        ...
        <div class="meta">
          ...
          ${(()=>{ const sp=r.skillPath||{}; const greenCount = SKILL_PATH_ITEMS.filter(it=>sp[it.key]==='green').length; const redCount = SKILL_PATH_ITEMS.filter(it=>sp[it.key]==='red').length; return greenCount||redCount ? `<span>🧭 путь: ${greenCount}/${SKILL_PATH_ITEMS.length}${redCount?` · 🔴 ${redCount}`:''}</span>` : ''; })()}
          ${(()=>{ const pending = (r.homework||[]).filter(h=>!h.done).length; return pending ? `<span>📋 д/з: ${pending}</span>` : ''; })()}
        ...
        <div class="edit-box ${editingIndex===i?'open':''}" data-box="${i}">
          ...
          <label style="margin-top:10px;">Путь спортсмена — освоение базовых навыков</label>
          ${skillPathBlockHtml(r)}
          <div style="font-size:11px;color:var(--dim);margin-top:4px;">Нажимайте на значок, чтобы менять статус: ⚪ не изучалось → 🔴 проблема → 🟡 в процессе → 🟢 освоено</div>

          <label style="margin-top:10px;">Домашнее задание</label>
          <div class="hw-block" data-hwslot="${i}">${homeworkBlockHtml(r, i)}</div>
    ...
    el.querySelectorAll('.skill-badge').forEach(badge=>{
      badge.addEventListener('click', ()=>{
        const cur = badge.dataset.state || 'none';
        const next = SKILL_PATH_STATES[(SKILL_PATH_STATES.indexOf(cur)+1) % SKILL_PATH_STATES.length];
        badge.dataset.state = next;
        badge.querySelector('span').textContent = SKILL_PATH_ICON[next];
        const item = SKILL_PATH_ITEMS.find(x=>x.key===badge.dataset.skill);
        badge.title = `${item.label} — ${SKILL_PATH_TITLE[next]}`;
      });
    });
    async function addHomework(i, text){
      text = text.trim();
      if(!text) return;
      const list = await getRoster();
      list[i].homework = list[i].homework || [];
      list[i].homework.push({id: uid(), date: new Date().toLocaleDateString('ru-RU'), text, done:false});
      await setRoster(list);
      editingIndex = i;
      renderRoster();
    }
    el.querySelectorAll('.hw-preset').forEach(b=>{
      b.addEventListener('click', ()=> addHomework(Number(b.dataset.i), b.dataset.text));
    });
    el.querySelectorAll('.hw-add-custom').forEach(b=>{
      b.addEventListener('click', ()=>{
        const i = Number(b.dataset.i);
        const input = el.querySelector(`.hw-custom[data-i="${i}"]`);
        addHomework(i, input.value);
      });
    });
    el.querySelectorAll('.hw-check').forEach(cb=>{
      cb.addEventListener('change', async ()=>{
        const i = Number(cb.dataset.i);
        const list = await getRoster();
        const hw = (list[i].homework||[]).find(h=>h.id===cb.dataset.hid);
        if(hw) hw.done = cb.checked;
        await setRoster(list);
        editingIndex = i;
        renderRoster();
      });
    });
    el.querySelectorAll('.hw-del').forEach(b=>{
      b.addEventListener('click', async ()=>{
        const i = Number(b.dataset.i);
        const list = await getRoster();
        list[i].homework = (list[i].homework||[]).filter(h=>h.id!==b.dataset.hid);
        await setRoster(list);
        editingIndex = i;
        renderRoster();
      });
    });
    el.querySelectorAll('.radar-chip').forEach(chip=>{
      chip.addEventListener('click', ()=>{
        const key = chip.dataset.radar;
        radarFilter = radarFilter===key ? null : key;
        renderRoster();
      });
    });
    const radarClearBtn = el.querySelector('.radar-clear');
    if(radarClearBtn) radarClearBtn.addEventListener('click', ()=>{ radarFilter = null; renderRoster(); });
    ...
    el.querySelectorAll('.e-save').forEach(b=>{
      b.addEventListener('click', async ()=>{
        ...
        const skillPath = {};
        box.querySelectorAll('.skill-badge').forEach(b=>{
          if(b.dataset.state && b.dataset.state!=='none') skillPath[b.dataset.skill] = b.dataset.state;
        });
        list[i].skillPath = skillPath;

...
У стенки тренируем подхват` }
  ...
  // ================= АНАЛИЗ ИСТОРИИ ТРЕНИРОВОК (анти-повторитель + "что мы забыли") =================
  // Работает по блокам конструктора, сохранённым в план дня. Данные накапливаются
  // постепенно — чем больше тренировок собрано через конструктор, тем точнее анализ.
  async function getRecentSessionsWithBlocks(daysBack){
    ...
    try{
      ...
      for(const k of keys){
        const dPart = k.slice('plan:'.length);
        if(dPart < cutoffKey) continue;
        try{
          const r = await S.get(k);
          if(!r) continue;
          const sessions = JSON.parse(r.value) || [];
          sessions.forEach(s=>{ if(s.blocks && s.blocks.length) results.push({date: dPart, blocks: s.blocks}); });
        }catch(e){}
      }
    }catch(e){}
    return results;
  }
  function blockSimilarity(blocksA, blocksB){
    if(!blocksA.length || !blocksB.length) return 0;
    const pool = blocksB.map(b=>`${b.type}::${(b.title||'').trim().toLowerCase()}`);
    let matches = 0;
    blocksA.forEach(a=>{
    ...
    return matches / Math.max(blocksA.length, blocksB.length);
  }
  async function checkAntiRepeat(){
    const hint = document.getElementById('constructor-similarity-hint');
    if(!hint) return;
    if(constructorBlocks.length < 2){ hint.style.display='none'; return; }
    const recent = await getRecentSessionsWithBlocks(21);
    let best = null;
    recent.forEach(r=>{
      const sim = blockSimilarity(constructorBlocks, r.blocks);
      if(!best || sim>best.sim) best = {sim, date:r.date};
    });
    if(best && best.sim >= 0.6){
      const pct = Math.round(best.sim*100);
      const niceDate = new Date(best.date).toLocaleDateString('ru-RU');
      hint.style.display = 'block';
      hint.innerHTML = `⚠️ Эта тренировка на ${pct}% похожа на тренировку от ${niceDate}. Попробуйте изменить разминку, игру или порядок техник.`;
    } else {
      hint.style.display = 'none';
    }
  }
  async function renderForgottenInsight(){
    const el = document.getElementById('constructor-forgotten-hint');
    if(!el) return;
    const recent = await getRecentSessionsWithBlocks(45);
    if(recent.length < 3){ el.innerHTML = '<div class="empty-hint">Пока мало данных для анализа — накопится после нескольких тренировок, собранных через конструктор.</div>'; return; }
    const lastUsed = {}; // type -> дата последнего использования (строка YYYY-MM-DD)
    recent.forEach(r=> r.blocks.forEach(b=>{
      if(!lastUsed[b.type] || r.date > lastUsed[b.type]) lastUsed[b.type] = r.date;
    }));
    const today = dateKey(new Date());
    const daysSince = (d)=> Math.round((new Date(today) - new Date(d)) / 86400000);
    const relevantTypes = BLOCK_TYPES.filter(t=>t.key!=='warmup' && t.key!=='cooldown');
    const stale = relevantTypes.map(t=>({
      type:t, days: lastUsed[t.key] ? daysSince(lastUsed[t.key]) : null
    })).filter(x=> x.days===null || x.days>=14).sort((a,b)=> (b.days??999)-(a.days??999));
    if(!stale.length){ el.innerHTML = '<div class="empty-hint">За последние недели все разделы тренировки встречались регулярно 👍</div>'; return; }
    el.innerHTML = stale.map(x=> `<div style="padding:4px 0;">⚠️ <b>${x.type.icon} ${x.type.label}</b> — ${x.days===null ? 'не встречалось в последних тренировках' : `последний раз ${x.days} дн. назад`}</div>`).join('');
  }
  ...
  document.getElementById('constructor-apply-today').addEventListener('click', async ()=>{
    ...
    target.blocks = JSON.parse(JSON.stringify(constructorBlocks)); // для анти-повторителя и анализа "что мы забыли"

  ...
  const MIGRATIONS = {
    // Пока миграций нет — это стартовая версия схемы данных.
    // Пример на будущее:
    // 2: async function(){
    //   const list = JSON.parse((await rawGet('roster')) || '[]');
    //   list.forEach(s => { if(s.someOldField !== undefined && s.someNewField === undefined) s.someNewField = s.someOldField; });
    //   await rawSet('roster', JSON.stringify(list));
    // }
  };

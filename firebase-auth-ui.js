/* Judo Coach — Email/Password UI, без клубов и ролей.
...
(() => {
  ...
  function errorText(e) {
    const map = {
      ...
      'auth/network-request-failed': 'Нет соединения с сервером. Проверьте интернет и адрес backend.',
  ...
  async function init() {
    ...
    $('auth-migration-password-btn')?.addEventListener('click', async () => {
      ...
      if (!user?.email) { message('Текущий аккаунт не найден.', 'error'); return; }

...
(function(){
  ...
  function attach(){
    try {
      if (!window.TERMINOLOGY_DATA && typeof TERMINOLOGY_DATA !== 'undefined') window.TERMINOLOGY_DATA = TERMINOLOGY_DATA;
      const data = window.TERMINOLOGY_DATA;
      if (!data || !Array.isArray(data.techniques)) return false;
      let count = 0;
      data.techniques.forEach(function(t){
        const id = DRIVE_TECHNIQUE_VIDEOS[t.romaji];
        if (id) { t.video2 = 'https://drive.google.com/file/d/' + id + '/view'; count++; }
      });
